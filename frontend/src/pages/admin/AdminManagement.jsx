...
import { FiUserPlus, FiEdit2, FiTrash2, FiShield, FiMoreVertical, FiCheck, FiX } from 'react-icons/fi';
import { superAdminAPI } from '../../api/superAdmin';
import { useAuthStore } from '../../store/authStore';
import './AdminPages.css'; // Reuse existing styles or create new
...
export default function AdminManagement() {
    ...
    const openRoleModal = (admin) => {
        ...
        // Map current roles (if backend provided them, otherwise we assume none for now)
        // Note: The backend getAdmins might not populate 'roles' fully yet unless we verified it.
        // Assuming we update backend to include 'roles' list in to_dict or we fetch specific.
        // For simplicity, let's start with empty or check update.
        // WAIT: The regular User.to_dict doesn't explicitly list granular roles. 
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto animate-in fade-in duration-500">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    <h2 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">System Administrators</h2>
                    <p className="text-slate-500 dark:text-slate-400">Manage access and permissions for platform administrators.</p>
                </div>
                <button
                    ...
                    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl w-full max-w-md p-6 shadow-2xl animate-in zoom-in-95 duration-200">
                        ...
                        <form onSubmit={handleCreate} className="space-y-4">
                            ...
                            <div className="flex justify-end gap-3 mt-8">
                                ...
                                <button
                                    type="submit"
                                    className="px-4 py-2 bg-indigo-600 text-white font-bold rounded-lg hover:bg-indigo-500 transition-colors shadow-lg shadow-indigo-900/20"
                                >
                                    Create Admin
                                </button>
                            </div>
                            ...
                            {roles.map(role => (
                                <label key={role.id} className="flex items-start gap-3 p-3 rounded-lg border border-slate-700 hover:bg-slate-700/50 cursor-pointer transition-colors group">
                                    <input
                                        ...
                                        className="mt-1 w-4 h-4 rounded bg-slate-900 border-slate-600 text-indigo-600 focus:ring-indigo-500"
                                    />
                                    <div>
